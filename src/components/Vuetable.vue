        ...
        callCallback: function(field, item) {
            ...
            if (typeof this.$parent[func] == 'function') {
                return (args.length > 0)
                    ? this.$parent[func].apply(this.$parent, [item, this.getObjectValue(item, field.name)].concat(args))
                    : this.$parent[func].apply(this.$parent, [item, this.getObjectValue(item, field.name)])
            }
